...
Uso:
  ...
  ./dev.sh --profile dev  Avvia il backend con profilo dev
  ./dev.sh --profile local  Avvia il backend con profilo local
  ./dev.sh --fe-port 4201   Avvia Angular su una porta specifica
...
Note:
- Backend: ./ordering-system/mvnw spring-boot:run (profilo predefinito: local)
...
while [ "$#" -gt 0 ]; do
  ...
  case "$arg" in
    ...
    --profile)
      if [ "$#" -eq 0 ]; then
        echo "Manca il valore per --profile" >&2
        exit 2
      fi
      case "$1" in
        local|dev) SPRING_PROFILE="$1" ;;
        *) echo "Profilo non riconosciuto: $1" >&2; exit 2 ;;
      esac
      shift
      ;;
    --fe-port)
      if [ "$#" -eq 0 ]; then
        echo "Manca il valore per --fe-port" >&2
        exit 2
      fi
      FE_PORT="$1"
      shift
...
(
  cd "$BACKEND_DIR"
  exec ./mvnw spring-boot:run -Dspring-boot.run.profiles="$SPRING_PROFILE"
) &
BE_PID=$!
...
(
  ...
  if port_in_use "$FE_PORT"; then
    if [ "$FE_PORT" != "4200" ]; then
      echo "[FE] Porta $FE_PORT già in uso. Scegli una porta libera con --fe-port." >&2
      exit 1
    fi
    FREE_PORT="$(pick_free_port 4201 4210 || true)"
    if [ -z "${FREE_PORT:-}" ]; then
      echo "[FE] Nessuna porta libera trovata tra 4201 e 4210. Libera una porta o usa --fe-port." >&2
      exit 1
    fi
    FE_PORT="$FREE_PORT"
    echo "[FE] Porta 4200 occupata: uso $FE_PORT"
  fi

  exec npm start -- --port "$FE_PORT"
) &
FE_PID=$!
